...
{
	# Initialize
	var_ranef_Race.TrackTypeCd_dim_row<-dim(jackpot_results_glmer_ranef$Race.TrackTypeCd)[1]
	var_ranef_Race.TrackTypeCd_dim_col<-dim(jackpot_results_glmer_ranef$Race.TrackTypeCd)[2]
	...
	{
		# Col : WHEN
		var_ranef_Race.TrackTypeCd_line_when <- paste0(const_case_when_Race.TrackTypeCd, rownames(jackpot_results_glmer_ranef$Race.TrackTypeCd)[row])
		
		# Col : THEN
		var_ranef_Race.TrackTypeCd_line_then <- const_case_then
		for(col in 1: var_ranef_Race.TrackTypeCd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$Race.TrackTypeCd[col]),nomatch=0) > 0)
			{
				var_ranef_Race.TrackTypeCd_line_name <- colnames(jackpot_results_glmer_ranef$Race.TrackTypeCd[col])
				var_ranef_Race.TrackTypeCd_line_value <- as.character(jackpot_results_glmer_ranef$Race.TrackTypeCd[row,col])
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$Race.JyokenGradeCd)>0)
{
	# Initialize
	var_ranef_Race.JyokenGradeCd_dim_row<-dim(jackpot_results_glmer_ranef$Race.JyokenGradeCd)[1]
	var_ranef_Race.JyokenGradeCd_dim_col<-dim(jackpot_results_glmer_ranef$Race.JyokenGradeCd)[2]
	...
	{
		# Col : WHEN
		var_ranef_Race.JyokenGradeCd_line_when <- paste0(const_case_when_Race.JyokenGradeCd, rownames(jackpot_results_glmer_ranef$Race.JyokenGradeCd)[row], const_case_when_single_quote)
		
		# Col : THEN
		var_ranef_Race.JyokenGradeCd_line_then <- const_case_then
		for(col in 1: var_ranef_Race.JyokenGradeCd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$Race.JyokenGradeCd[col]),nomatch=0) > 0)
			{
				var_ranef_Race.JyokenGradeCd_line_name <- colnames(jackpot_results_glmer_ranef$Race.JyokenGradeCd[col])
				var_ranef_Race.JyokenGradeCd_line_value <- as.character(jackpot_results_glmer_ranef$Race.JyokenGradeCd[row,col])
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$Race.S38Avg)>0)
{
	# Initialize
	var_ranef_Race.S38Avg_dim_row<-dim(jackpot_results_glmer_ranef$Race.S38Avg)[1]
	var_ranef_Race.S38Avg_dim_col<-dim(jackpot_results_glmer_ranef$Race.S38Avg)[2]
	...
	{
		# Col : WHEN
		var_ranef_Race.S38Avg_line_when <- paste0(const_case_when_Race.S38Avg, rownames(jackpot_results_glmer_ranef$Race.S38Avg)[row])
		
		# Col : THEN
		var_ranef_Race.S38Avg_line_then <- const_case_then
		for(col in 1: var_ranef_Race.S38Avg_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$Race.S38Avg[col]),nomatch=0) > 0)
			{
				var_ranef_Race.S38Avg_line_name <- colnames(jackpot_results_glmer_ranef$Race.S38Avg[col])
				var_ranef_Race.S38Avg_line_value <- as.character(jackpot_results_glmer_ranef$Race.S38Avg[row,col])
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$Race.SmileCd)>0)
{
	# Initialize
	var_ranef_Race.SmileCd_dim_row<-dim(jackpot_results_glmer_ranef$Race.SmileCd)[1]
	var_ranef_Race.SmileCd_dim_col<-dim(jackpot_results_glmer_ranef$Race.SmileCd)[2]
	...
	{
		# Col : WHEN
		var_ranef_Race.SmileCd_line_when <- paste0(const_case_when_Race.SmileCd, rownames(jackpot_results_glmer_ranef$Race.SmileCd)[row], const_case_when_single_quote)
		
		# Col : THEN
		var_ranef_Race.SmileCd_line_then <- const_case_then
		for(col in 1: var_ranef_Race.SmileCd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$Race.SmileCd[col]),nomatch=0) > 0)
			{
				var_ranef_Race.SmileCd_line_name <- colnames(jackpot_results_glmer_ranef$Race.SmileCd[col])
				var_ranef_Race.SmileCd_line_value <- as.character(jackpot_results_glmer_ranef$Race.SmileCd[row,col])
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$Race.RaceType2Cd)>0)
{
	# Initialize
	var_ranef_Race.RaceType2Cd_dim_row<-dim(jackpot_results_glmer_ranef$Race.RaceType2Cd)[1]
	var_ranef_Race.RaceType2Cd_dim_col<-dim(jackpot_results_glmer_ranef$Race.RaceType2Cd)[2]
	...
	{
		# Col : WHEN
		var_ranef_Race.RaceType2Cd_line_when <- paste0(const_case_when_Race.RaceType2Cd, rownames(jackpot_results_glmer_ranef$Race.RaceType2Cd)[row], const_case_when_single_quote)
		
		# Col : THEN
		var_ranef_Race.RaceType2Cd_line_then <- const_case_then
		for(col in 1: var_ranef_Race.RaceType2Cd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$Race.RaceType2Cd[col]),nomatch=0) > 0)
			{
				var_ranef_Race.RaceType2Cd_line_name <- colnames(jackpot_results_glmer_ranef$Race.RaceType2Cd[col])
				var_ranef_Race.RaceType2Cd_line_value <- as.character(jackpot_results_glmer_ranef$Race.RaceType2Cd[row,col])
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$Race.RaceType3Cd)>0)
{
	# Initialize
	var_ranef_Race.RaceType3Cd_dim_row<-dim(jackpot_results_glmer_ranef$Race.RaceType3Cd)[1]
	var_ranef_Race.RaceType3Cd_dim_col<-dim(jackpot_results_glmer_ranef$Race.RaceType3Cd)[2]
	...
	{
		# Col : WHEN
		var_ranef_Race.RaceType3Cd_line_when <- paste0(const_case_when_Race.RaceType3Cd, rownames(jackpot_results_glmer_ranef$Race.RaceType3Cd)[row], const_case_when_single_quote)
		
		# Col : THEN
		var_ranef_Race.RaceType3Cd_line_then <- const_case_then
		for(col in 1: var_ranef_Race.RaceType3Cd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$Race.RaceType3Cd[col]),nomatch=0) > 0)
			{
				var_ranef_Race.RaceType3Cd_line_name <- colnames(jackpot_results_glmer_ranef$Race.RaceType3Cd[col])
				var_ranef_Race.RaceType3Cd_line_value <- as.character(jackpot_results_glmer_ranef$Race.RaceType3Cd[row,col])
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$Race.RaceType4Cd)>0)
{
	# Initialize
	var_ranef_Race.RaceType4Cd_dim_row<-dim(jackpot_results_glmer_ranef$Race.RaceType4Cd)[1]
	var_ranef_Race.RaceType4Cd_dim_col<-dim(jackpot_results_glmer_ranef$Race.RaceType4Cd)[2]
	...
	{
		# Col : WHEN
		var_ranef_Race.RaceType4Cd_line_when <- paste0(const_case_when_Race.RaceType4Cd, rownames(jackpot_results_glmer_ranef$Race.RaceType4Cd)[row], const_case_when_single_quote)
		
		# Col : THEN
		var_ranef_Race.RaceType4Cd_line_then <- const_case_then
		for(col in 1: var_ranef_Race.RaceType4Cd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$Race.RaceType4Cd[col]),nomatch=0) > 0)
			{
				var_ranef_Race.RaceType4Cd_line_name <- colnames(jackpot_results_glmer_ranef$Race.RaceType4Cd[col])
				var_ranef_Race.RaceType4Cd_line_value <- as.character(jackpot_results_glmer_ranef$Race.RaceType4Cd[row,col])
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$Race.RaceType5Cd)>0)
{
	# Initialize
	var_ranef_Race.RaceType5Cd_dim_row<-dim(jackpot_results_glmer_ranef$Race.RaceType5Cd)[1]
	var_ranef_Race.RaceType5Cd_dim_col<-dim(jackpot_results_glmer_ranef$Race.RaceType5Cd)[2]
	...
	{
		# Col : WHEN
		var_ranef_Race.RaceType5Cd_line_when <- paste0(const_case_when_Race.RaceType5Cd, rownames(jackpot_results_glmer_ranef$Race.RaceType5Cd)[row], const_case_when_single_quote)
		
		# Col : THEN
		var_ranef_Race.RaceType5Cd_line_then <- const_case_then
		for(col in 1: var_ranef_Race.RaceType5Cd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$Race.RaceType5Cd[col]),nomatch=0) > 0)
			{
				var_ranef_Race.RaceType5Cd_line_name <- colnames(jackpot_results_glmer_ranef$Race.RaceType5Cd[col])
				var_ranef_Race.RaceType5Cd_line_value <- as.character(jackpot_results_glmer_ranef$Race.RaceType5Cd[row,col])
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$Race.RaceType6Cd)>0)
{
	# Initialize
	var_ranef_Race.RaceType6Cd_dim_row<-dim(jackpot_results_glmer_ranef$Race.RaceType6Cd)[1]
	var_ranef_Race.RaceType6Cd_dim_col<-dim(jackpot_results_glmer_ranef$Race.RaceType6Cd)[2]
	...
	{
		# Col : WHEN
		var_ranef_Race.RaceType6Cd_line_when <- paste0(const_case_when_Race.RaceType6Cd, rownames(jackpot_results_glmer_ranef$Race.RaceType6Cd)[row], const_case_when_single_quote)
		
		# Col : THEN
		var_ranef_Race.RaceType6Cd_line_then <- const_case_then
		for(col in 1: var_ranef_Race.RaceType6Cd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$Race.RaceType6Cd[col]),nomatch=0) > 0)
			{
				var_ranef_Race.RaceType6Cd_line_name <- colnames(jackpot_results_glmer_ranef$Race.RaceType6Cd[col])
				var_ranef_Race.RaceType6Cd_line_value <- as.character(jackpot_results_glmer_ranef$Race.RaceType6Cd[row,col])
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$Race.RaceType7Cd)>0)
{
	# Initialize
	var_ranef_Race.RaceType7Cd_dim_row<-dim(jackpot_results_glmer_ranef$Race.RaceType7Cd)[1]
	var_ranef_Race.RaceType7Cd_dim_col<-dim(jackpot_results_glmer_ranef$Race.RaceType7Cd)[2]
	...
	{
		# Col : WHEN
		var_ranef_Race.RaceType7Cd_line_when <- paste0(const_case_when_Race.RaceType7Cd, rownames(jackpot_results_glmer_ranef$Race.RaceType7Cd)[row], const_case_when_single_quote)
		
		# Col : THEN
		var_ranef_Race.RaceType7Cd_line_then <- const_case_then
		for(col in 1: var_ranef_Race.RaceType7Cd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$Race.RaceType7Cd[col]),nomatch=0) > 0)
			{
				var_ranef_Race.RaceType7Cd_line_name <- colnames(jackpot_results_glmer_ranef$Race.RaceType7Cd[col])
				var_ranef_Race.RaceType7Cd_line_value <- as.character(jackpot_results_glmer_ranef$Race.RaceType7Cd[row,col])
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType2Cd)>0)
{
	# Initialize
	var_ranef_RaceHorse.PrevRaceType2Cd_dim_row<-dim(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType2Cd)[1]
	var_ranef_RaceHorse.PrevRaceType2Cd_dim_col<-dim(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType2Cd)[2]
	...
	{
		# Col : WHEN
		var_ranef_RaceHorse.PrevRaceType2Cd_line_when <- paste0(const_case_when_RaceHorse.PrevRaceType2Cd, rownames(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType2Cd)[row], const_case_when_single_quote)
		
		# Col : THEN
		var_ranef_RaceHorse.PrevRaceType2Cd_line_then <- const_case_then
		for(col in 1: var_ranef_RaceHorse.PrevRaceType2Cd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType2Cd[col]),nomatch=0) > 0)
			{
				var_ranef_RaceHorse.PrevRaceType2Cd_line_name <- colnames(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType2Cd[col])
				var_ranef_RaceHorse.PrevRaceType2Cd_line_value <- as.character(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType2Cd[row,col])
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType3Cd)>0)
{
	# Initialize
	var_ranef_RaceHorse.PrevRaceType3Cd_dim_row<-dim(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType3Cd)[1]
	var_ranef_RaceHorse.PrevRaceType3Cd_dim_col<-dim(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType3Cd)[2]
	...
	{
		# Col : WHEN
		var_ranef_RaceHorse.PrevRaceType3Cd_line_when <- paste0(const_case_when_RaceHorse.PrevRaceType3Cd, rownames(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType3Cd)[row], const_case_when_single_quote)
		
		# Col : THEN
		var_ranef_RaceHorse.PrevRaceType3Cd_line_then <- const_case_then
		for(col in 1: var_ranef_RaceHorse.PrevRaceType3Cd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType3Cd[col]),nomatch=0) > 0)
			{
				var_ranef_RaceHorse.PrevRaceType3Cd_line_name <- colnames(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType3Cd[col])
				var_ranef_RaceHorse.PrevRaceType3Cd_line_value <- as.character(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType3Cd[row,col])
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType4Cd)>0)
{
	# Initialize
	var_ranef_RaceHorse.PrevRaceType4Cd_dim_row<-dim(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType4Cd)[1]
	var_ranef_RaceHorse.PrevRaceType4Cd_dim_col<-dim(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType4Cd)[2]
	...
	{
		# Col : WHEN
		var_ranef_RaceHorse.PrevRaceType4Cd_line_when <- paste0(const_case_when_RaceHorse.PrevRaceType4Cd, rownames(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType4Cd)[row], const_case_when_single_quote)
		
		# Col : THEN
		var_ranef_RaceHorse.PrevRaceType4Cd_line_then <- const_case_then
		for(col in 1: var_ranef_RaceHorse.PrevRaceType4Cd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType4Cd[col]),nomatch=0) > 0)
			{
				var_ranef_RaceHorse.PrevRaceType4Cd_line_name <- colnames(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType4Cd[col])
				var_ranef_RaceHorse.PrevRaceType4Cd_line_value <- as.character(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType4Cd[row,col])
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType5Cd)>0)
{
	# Initialize
	var_ranef_RaceHorse.PrevRaceType5Cd_dim_row<-dim(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType5Cd)[1]
	var_ranef_RaceHorse.PrevRaceType5Cd_dim_col<-dim(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType5Cd)[2]
	...
	{
		# Col : WHEN
		var_ranef_RaceHorse.PrevRaceType5Cd_line_when <- paste0(const_case_when_RaceHorse.PrevRaceType5Cd, rownames(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType5Cd)[row], const_case_when_single_quote)
		
		# Col : THEN
		var_ranef_RaceHorse.PrevRaceType5Cd_line_then <- const_case_then
		for(col in 1: var_ranef_RaceHorse.PrevRaceType5Cd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType5Cd[col]),nomatch=0) > 0)
			{
				var_ranef_RaceHorse.PrevRaceType5Cd_line_name <- colnames(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType5Cd[col])
				var_ranef_RaceHorse.PrevRaceType5Cd_line_value <- as.character(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType5Cd[row,col])
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType6Cd)>0)
{
	# Initialize
	var_ranef_RaceHorse.PrevRaceType6Cd_dim_row<-dim(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType6Cd)[1]
	var_ranef_RaceHorse.PrevRaceType6Cd_dim_col<-dim(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType6Cd)[2]
	...
	{
		# Col : WHEN
		var_ranef_RaceHorse.PrevRaceType6Cd_line_when <- paste0(const_case_when_RaceHorse.PrevRaceType6Cd, rownames(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType6Cd)[row], const_case_when_single_quote)
		
		# Col : THEN
		var_ranef_RaceHorse.PrevRaceType6Cd_line_then <- const_case_then
		for(col in 1: var_ranef_RaceHorse.PrevRaceType6Cd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType6Cd[col]),nomatch=0) > 0)
			{
				var_ranef_RaceHorse.PrevRaceType6Cd_line_name <- colnames(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType6Cd[col])
				var_ranef_RaceHorse.PrevRaceType6Cd_line_value <- as.character(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType6Cd[row,col])
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType7Cd)>0)
{
	# Initialize
	var_ranef_RaceHorse.PrevRaceType7Cd_dim_row<-dim(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType7Cd)[1]
	var_ranef_RaceHorse.PrevRaceType7Cd_dim_col<-dim(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType7Cd)[2]
	...
	{
		# Col : WHEN
		var_ranef_RaceHorse.PrevRaceType7Cd_line_when <- paste0(const_case_when_RaceHorse.PrevRaceType7Cd, rownames(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType7Cd)[row], const_case_when_single_quote)
		
		# Col : THEN
		var_ranef_RaceHorse.PrevRaceType7Cd_line_then <- const_case_then
		for(col in 1: var_ranef_RaceHorse.PrevRaceType7Cd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType7Cd[col]),nomatch=0) > 0)
			{
				var_ranef_RaceHorse.PrevRaceType7Cd_line_name <- colnames(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType7Cd[col])
				var_ranef_RaceHorse.PrevRaceType7Cd_line_value <- as.character(jackpot_results_glmer_ranef$RaceHorse.PrevRaceType7Cd[row,col])
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$PrevRace.RaceType2Cd)>0)
{
	# Initialize
	var_ranef_Race.RaceType2Cd_dim_row<-dim(jackpot_results_glmer_ranef$PrevRace.RaceType2Cd)[1]
	var_ranef_Race.RaceType2Cd_dim_col<-dim(jackpot_results_glmer_ranef$PrevRace.RaceType2Cd)[2]
	var_ranef_Race.RaceType2Cd_matrix <- NULL
	...
	{
		# Col : WHEN
		var_ranef_Race.RaceType2Cd_line_when <- paste0(const_case_when_Race.RaceType2Cd, rownames(jackpot_results_glmer_ranef$PrevRace.RaceType2Cd)[row], const_case_when_single_quote)
		
		# Col : THEN
		var_ranef_Race.RaceType2Cd_line_then <- const_case_then
		for(col in 1: var_ranef_Race.RaceType2Cd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$PrevRace.RaceType2Cd[col]),nomatch=0) > 0)
			{
				var_ranef_Race.RaceType2Cd_line_name <- colnames(jackpot_results_glmer_ranef$PrevRace.RaceType2Cd[col])
				var_ranef_Race.RaceType2Cd_line_value <- as.character(jackpot_results_glmer_ranef$PrevRace.RaceType2Cd[row,col])
				var_ranef_Race.RaceType2Cd_line_then <- paste0(var_ranef_Race.RaceType2Cd_line_then,const_ope_plus,const_convert_open,var_ranef_Race.RaceType2Cd_line_name,const_convert_close,const_ope_multi,var_ranef_Race.RaceType2Cd_line_value)
			}
		}
		# Col
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$PrevRace.RaceType3Cd)>0)
{
	# Initialize
	var_ranef_Race.RaceType3Cd_dim_row<-dim(jackpot_results_glmer_ranef$PrevRace.RaceType3Cd)[1]
	var_ranef_Race.RaceType3Cd_dim_col<-dim(jackpot_results_glmer_ranef$PrevRace.RaceType3Cd)[2]
	var_ranef_Race.RaceType3Cd_matrix <- NULL
	...
	{
		# Col : WHEN
		var_ranef_Race.RaceType3Cd_line_when <- paste0(const_case_when_Race.RaceType3Cd, rownames(jackpot_results_glmer_ranef$PrevRace.RaceType3Cd)[row], const_case_when_single_quote)
		
		# Col : THEN
		var_ranef_Race.RaceType3Cd_line_then <- const_case_then
		for(col in 1: var_ranef_Race.RaceType3Cd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$PrevRace.RaceType3Cd[col]),nomatch=0) > 0)
			{
				var_ranef_Race.RaceType3Cd_line_name <- colnames(jackpot_results_glmer_ranef$PrevRace.RaceType3Cd[col])
				var_ranef_Race.RaceType3Cd_line_value <- as.character(jackpot_results_glmer_ranef$PrevRace.RaceType3Cd[row,col])
				var_ranef_Race.RaceType3Cd_line_then <- paste0(var_ranef_Race.RaceType3Cd_line_then,const_ope_plus,const_convert_open,var_ranef_Race.RaceType3Cd_line_name,const_convert_close,const_ope_multi,var_ranef_Race.RaceType3Cd_line_value)
			}
		}
		# Col
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$PrevRace.RaceType4Cd)>0)
{
	# Initialize
	var_ranef_Race.RaceType4Cd_dim_row<-dim(jackpot_results_glmer_ranef$PrevRace.RaceType4Cd)[1]
	var_ranef_Race.RaceType4Cd_dim_col<-dim(jackpot_results_glmer_ranef$PrevRace.RaceType4Cd)[2]
	var_ranef_Race.RaceType4Cd_matrix <- NULL
	...
	{
		# Col : WHEN
		var_ranef_Race.RaceType4Cd_line_when <- paste0(const_case_when_Race.RaceType4Cd, rownames(jackpot_results_glmer_ranef$PrevRace.RaceType4Cd)[row], const_case_when_single_quote)
		
		# Col : THEN
		var_ranef_Race.RaceType4Cd_line_then <- const_case_then
		for(col in 1: var_ranef_Race.RaceType4Cd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$PrevRace.RaceType4Cd[col]),nomatch=0) > 0)
			{
				var_ranef_Race.RaceType4Cd_line_name <- colnames(jackpot_results_glmer_ranef$PrevRace.RaceType4Cd[col])
				var_ranef_Race.RaceType4Cd_line_value <- as.character(jackpot_results_glmer_ranef$PrevRace.RaceType4Cd[row,col])
				var_ranef_Race.RaceType4Cd_line_then <- paste0(var_ranef_Race.RaceType4Cd_line_then,const_ope_plus,const_convert_open,var_ranef_Race.RaceType4Cd_line_name,const_convert_close,const_ope_multi,var_ranef_Race.RaceType4Cd_line_value)
			}
		}
		# Col
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$PrevRace.RaceType5Cd)>0)
{
	# Initialize
	var_ranef_Race.RaceType5Cd_dim_row<-dim(jackpot_results_glmer_ranef$PrevRace.RaceType5Cd)[1]
	var_ranef_Race.RaceType5Cd_dim_col<-dim(jackpot_results_glmer_ranef$PrevRace.RaceType5Cd)[2]
	var_ranef_Race.RaceType5Cd_matrix <- NULL
	...
	{
		# Col : WHEN
		var_ranef_Race.RaceType5Cd_line_when <- paste0(const_case_when_Race.RaceType5Cd, rownames(jackpot_results_glmer_ranef$PrevRace.RaceType5Cd)[row], const_case_when_single_quote)
		
		# Col : THEN
		var_ranef_Race.RaceType5Cd_line_then <- const_case_then
		for(col in 1: var_ranef_Race.RaceType5Cd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$PrevRace.RaceType5Cd[col]),nomatch=0) > 0)
			{
				var_ranef_Race.RaceType5Cd_line_name <- colnames(jackpot_results_glmer_ranef$PrevRace.RaceType5Cd[col])
				var_ranef_Race.RaceType5Cd_line_value <- as.character(jackpot_results_glmer_ranef$PrevRace.RaceType5Cd[row,col])
				var_ranef_Race.RaceType5Cd_line_then <- paste0(var_ranef_Race.RaceType5Cd_line_then,const_ope_plus,const_convert_open,var_ranef_Race.RaceType5Cd_line_name,const_convert_close,const_ope_multi,var_ranef_Race.RaceType5Cd_line_value)
			}
		}
		# Col
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$PrevRace.RaceType6Cd)>0)
{
	# Initialize
	var_ranef_Race.RaceType6Cd_dim_row<-dim(jackpot_results_glmer_ranef$PrevRace.RaceType6Cd)[1]
	var_ranef_Race.RaceType6Cd_dim_col<-dim(jackpot_results_glmer_ranef$PrevRace.RaceType6Cd)[2]
	var_ranef_Race.RaceType6Cd_matrix <- NULL
	...
	{
		# Col : WHEN
		var_ranef_Race.RaceType6Cd_line_when <- paste0(const_case_when_Race.RaceType6Cd, rownames(jackpot_results_glmer_ranef$PrevRace.RaceType6Cd)[row], const_case_when_single_quote)
		
		# Col : THEN
		var_ranef_Race.RaceType6Cd_line_then <- const_case_then
		for(col in 1: var_ranef_Race.RaceType6Cd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$PrevRace.RaceType6Cd[col]),nomatch=0) > 0)
			{
				var_ranef_Race.RaceType6Cd_line_name <- colnames(jackpot_results_glmer_ranef$PrevRace.RaceType6Cd[col])
				var_ranef_Race.RaceType6Cd_line_value <- as.character(jackpot_results_glmer_ranef$PrevRace.RaceType6Cd[row,col])
				var_ranef_Race.RaceType6Cd_line_then <- paste0(var_ranef_Race.RaceType6Cd_line_then,const_ope_plus,const_convert_open,var_ranef_Race.RaceType6Cd_line_name,const_convert_close,const_ope_multi,var_ranef_Race.RaceType6Cd_line_value)
			}
		}
		# Col
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$PrevRace.RaceType7Cd)>0)
{
	# Initialize
	var_ranef_Race.RaceType7Cd_dim_row<-dim(jackpot_results_glmer_ranef$PrevRace.RaceType7Cd)[1]
	var_ranef_Race.RaceType7Cd_dim_col<-dim(jackpot_results_glmer_ranef$PrevRace.RaceType7Cd)[2]
	var_ranef_Race.RaceType7Cd_matrix <- NULL
	...
	{
		# Col : WHEN
		var_ranef_Race.RaceType7Cd_line_when <- paste0(const_case_when_Race.RaceType7Cd, rownames(jackpot_results_glmer_ranef$PrevRace.RaceType7Cd)[row], const_case_when_single_quote)
		
		# Col : THEN
		var_ranef_Race.RaceType7Cd_line_then <- const_case_then
		for(col in 1: var_ranef_Race.RaceType7Cd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$PrevRace.RaceType7Cd[col]),nomatch=0) > 0)
			{
				var_ranef_Race.RaceType7Cd_line_name <- colnames(jackpot_results_glmer_ranef$PrevRace.RaceType7Cd[col])
				var_ranef_Race.RaceType7Cd_line_value <- as.character(jackpot_results_glmer_ranef$PrevRace.RaceType7Cd[row,col])
				var_ranef_Race.RaceType7Cd_line_then <- paste0(var_ranef_Race.RaceType7Cd_line_then,const_ope_plus,const_convert_open,var_ranef_Race.RaceType7Cd_line_name,const_convert_close,const_ope_multi,var_ranef_Race.RaceType7Cd_line_value)
			}
		}
		# Col
...
{
	# Initialize
	var_ranef_Race.JyoType1Cd_dim_row<-dim(jackpot_results_glmer_ranef$PrevRace.JyoType1Cd)[1]
	var_ranef_Race.JyoType1Cd_dim_col<-dim(jackpot_results_glmer_ranef$PrevRace.JyoType1Cd)[2]
	var_ranef_Race.JyoType1Cd_matrix <- NULL
	...
	{
		# Col : WHEN
		var_ranef_Race.JyoType1Cd_line_when <- paste0(const_case_when_Race.JyoType1Cd, rownames(jackpot_results_glmer_ranef$PrevRace.JyoType1Cd)[row], const_case_when_single_quote)
		...
		for(col in 1: var_ranef_Race.JyoType1Cd_dim_col)
		{
			...
			{
				var_ranef_Race.JyoType1Cd_line_name <- colnames(jackpot_results_glmer_ranef$PrevRace.JyoType1Cd[col])
				var_ranef_Race.JyoType1Cd_line_value <- as.character(jackpot_results_glmer_ranef$PrevRace.JyoType1Cd[row,col])
				var_ranef_Race.JyoType1Cd_line_then <- paste0(var_ranef_Race.JyoType1Cd_line_then,const_ope_plus,const_convert_open,var_ranef_Race.JyoType1Cd_line_name,const_convert_close,const_ope_multi,var_ranef_Race.JyoType1Cd_line_value)
			}
		}
		# Col
...
{
	# Initialize
	var_ranef_Race.RaceType0Cd_dim_row<-dim(jackpot_results_glmer_ranef$PrevRace.RaceType0Cd)[1]
	var_ranef_Race.RaceType0Cd_dim_col<-dim(jackpot_results_glmer_ranef$PrevRace.RaceType0Cd)[2]
	var_ranef_Race.RaceType0Cd_matrix <- NULL
	...
	{
		# Col : WHEN
		var_ranef_Race.RaceType0Cd_line_when <- paste0(const_case_when_Race.RaceType0Cd, rownames(jackpot_results_glmer_ranef$PrevRace.RaceType0Cd)[row], const_case_when_single_quote)
		...
		for(col in 1: var_ranef_Race.RaceType0Cd_dim_col)
		{
			...
			{
				var_ranef_Race.RaceType0Cd_line_name <- colnames(jackpot_results_glmer_ranef$PrevRace.RaceType0Cd[col])
				var_ranef_Race.RaceType0Cd_line_value <- as.character(jackpot_results_glmer_ranef$PrevRace.RaceType0Cd[row,col])
				var_ranef_Race.RaceType0Cd_line_then <- paste0(var_ranef_Race.RaceType0Cd_line_then,const_ope_plus,const_convert_open,var_ranef_Race.RaceType0Cd_line_name,const_convert_close,const_ope_multi,var_ranef_Race.RaceType0Cd_line_value)
			}
		}
		# Col
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType1Cd)>0)
{
	# Initialize
	var_ranef_PrevRaceHorse.RaceType1Cd_dim_row<-dim(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType1Cd)[1]
	var_ranef_PrevRaceHorse.RaceType1Cd_dim_col<-dim(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType1Cd)[2]
	...
	{
		# Col : WHEN
		var_ranef_PrevRaceHorse.RaceType1Cd_line_when <- paste0(const_case_when_PrevRaceHorse.RaceType1Cd, rownames(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType1Cd)[row], const_case_when_single_quote)
		
		# Col : THEN
		var_ranef_PrevRaceHorse.RaceType1Cd_line_then <- const_case_then
		for(col in 1: var_ranef_PrevRaceHorse.RaceType1Cd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType1Cd[col]),nomatch=0) > 0)
			{
				var_ranef_PrevRaceHorse.RaceType1Cd_line_name <- colnames(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType1Cd[col])
				var_ranef_PrevRaceHorse.RaceType1Cd_line_value <- as.character(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType1Cd[row,col])
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType2Cd)>0)
{
	# Initialize
	var_ranef_PrevRaceHorse.RaceType2Cd_dim_row<-dim(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType2Cd)[1]
	var_ranef_PrevRaceHorse.RaceType2Cd_dim_col<-dim(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType2Cd)[2]
	...
	{
		# Col : WHEN
		var_ranef_PrevRaceHorse.RaceType2Cd_line_when <- paste0(const_case_when_PrevRaceHorse.RaceType2Cd, rownames(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType2Cd)[row], const_case_when_single_quote)
		
		# Col : THEN
		var_ranef_PrevRaceHorse.RaceType2Cd_line_then <- const_case_then
		for(col in 1: var_ranef_PrevRaceHorse.RaceType2Cd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType2Cd[col]),nomatch=0) > 0)
			{
				var_ranef_PrevRaceHorse.RaceType2Cd_line_name <- colnames(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType2Cd[col])
				var_ranef_PrevRaceHorse.RaceType2Cd_line_value <- as.character(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType2Cd[row,col])
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType3Cd)>0)
{
	# Initialize
	var_ranef_PrevRaceHorse.RaceType3Cd_dim_row<-dim(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType3Cd)[1]
	var_ranef_PrevRaceHorse.RaceType3Cd_dim_col<-dim(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType3Cd)[2]
	...
	{
		# Col : WHEN
		var_ranef_PrevRaceHorse.RaceType3Cd_line_when <- paste0(const_case_when_PrevRaceHorse.RaceType3Cd, rownames(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType3Cd)[row], const_case_when_single_quote)
		
		# Col : THEN
		var_ranef_PrevRaceHorse.RaceType3Cd_line_then <- const_case_then
		for(col in 1: var_ranef_PrevRaceHorse.RaceType3Cd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType3Cd[col]),nomatch=0) > 0)
			{
				var_ranef_PrevRaceHorse.RaceType3Cd_line_name <- colnames(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType3Cd[col])
				var_ranef_PrevRaceHorse.RaceType3Cd_line_value <- as.character(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType3Cd[row,col])
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType4Cd)>0)
{
	# Initialize
	var_ranef_PrevRaceHorse.RaceType4Cd_dim_row<-dim(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType4Cd)[1]
	var_ranef_PrevRaceHorse.RaceType4Cd_dim_col<-dim(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType4Cd)[2]
	...
	{
		# Col : WHEN
		var_ranef_PrevRaceHorse.RaceType4Cd_line_when <- paste0(const_case_when_PrevRaceHorse.RaceType4Cd, rownames(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType4Cd)[row], const_case_when_single_quote)
		
		# Col : THEN
		var_ranef_PrevRaceHorse.RaceType4Cd_line_then <- const_case_then
		for(col in 1: var_ranef_PrevRaceHorse.RaceType4Cd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType4Cd[col]),nomatch=0) > 0)
			{
				var_ranef_PrevRaceHorse.RaceType4Cd_line_name <- colnames(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType4Cd[col])
				var_ranef_PrevRaceHorse.RaceType4Cd_line_value <- as.character(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType4Cd[row,col])
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType5Cd)>0)
{
	# Initialize
	var_ranef_PrevRaceHorse.RaceType5Cd_dim_row<-dim(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType5Cd)[1]
	var_ranef_PrevRaceHorse.RaceType5Cd_dim_col<-dim(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType5Cd)[2]
	...
	{
		# Col : WHEN
		var_ranef_PrevRaceHorse.RaceType5Cd_line_when <- paste0(const_case_when_PrevRaceHorse.RaceType5Cd, rownames(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType5Cd)[row], const_case_when_single_quote)
		
		# Col : THEN
		var_ranef_PrevRaceHorse.RaceType5Cd_line_then <- const_case_then
		for(col in 1: var_ranef_PrevRaceHorse.RaceType5Cd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType5Cd[col]),nomatch=0) > 0)
			{
				var_ranef_PrevRaceHorse.RaceType5Cd_line_name <- colnames(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType5Cd[col])
				var_ranef_PrevRaceHorse.RaceType5Cd_line_value <- as.character(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType5Cd[row,col])
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType6Cd)>0)
{
	# Initialize
	var_ranef_PrevRaceHorse.RaceType6Cd_dim_row<-dim(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType6Cd)[1]
	var_ranef_PrevRaceHorse.RaceType6Cd_dim_col<-dim(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType6Cd)[2]
	...
	{
		# Col : WHEN
		var_ranef_PrevRaceHorse.RaceType6Cd_line_when <- paste0(const_case_when_PrevRaceHorse.RaceType6Cd, rownames(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType6Cd)[row], const_case_when_single_quote)
		
		# Col : THEN
		var_ranef_PrevRaceHorse.RaceType6Cd_line_then <- const_case_then
		for(col in 1: var_ranef_PrevRaceHorse.RaceType6Cd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType6Cd[col]),nomatch=0) > 0)
			{
				var_ranef_PrevRaceHorse.RaceType6Cd_line_name <- colnames(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType6Cd[col])
				var_ranef_PrevRaceHorse.RaceType6Cd_line_value <- as.character(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType6Cd[row,col])
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType7Cd)>0)
{
	# Initialize
	var_ranef_PrevRaceHorse.RaceType7Cd_dim_row<-dim(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType7Cd)[1]
	var_ranef_PrevRaceHorse.RaceType7Cd_dim_col<-dim(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType7Cd)[2]
	...
	{
		# Col : WHEN
		var_ranef_PrevRaceHorse.RaceType7Cd_line_when <- paste0(const_case_when_PrevRaceHorse.RaceType7Cd, rownames(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType7Cd)[row], const_case_when_single_quote)
		
		# Col : THEN
		var_ranef_PrevRaceHorse.RaceType7Cd_line_then <- const_case_then
		for(col in 1: var_ranef_PrevRaceHorse.RaceType7Cd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType7Cd[col]),nomatch=0) > 0)
			{
				var_ranef_PrevRaceHorse.RaceType7Cd_line_name <- colnames(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType7Cd[col])
				var_ranef_PrevRaceHorse.RaceType7Cd_line_value <- as.character(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType7Cd[row,col])
...
#----------------------------------------------------------------------------------------------
# Random Effects (PrevRaceHorse.JyoType1Cd)
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$PrevRaceHorse.JyoType1Cd)>0)
{
	# Initialize
	var_ranef_PrevRaceHorse.JyoType1Cd_dim_row<-dim(jackpot_results_glmer_ranef$PrevRaceHorse.JyoType1Cd)[1]
	var_ranef_PrevRaceHorse.JyoType1Cd_dim_col<-dim(jackpot_results_glmer_ranef$PrevRaceHorse.JyoType1Cd)[2]
	var_ranef_PrevRaceHorse.JyoType1Cd_matrix <- NULL
	
	# CASE
	var_ranef_PrevRaceHorse.JyoType1Cd_row_sql <- cbind(const_case_case)
	var_ranef_PrevRaceHorse.JyoType1Cd_matrix <- rbind(var_ranef_PrevRaceHorse.JyoType1Cd_matrix,var_ranef_PrevRaceHorse.JyoType1Cd_row_sql)
	
	for (row in 1:var_ranef_PrevRaceHorse.JyoType1Cd_dim_row)
	{
		# Col : WHEN
		var_ranef_PrevRaceHorse.JyoType1Cd_line_when <- paste0(const_case_when_PrevRaceHorse.JyoType1Cd, rownames(jackpot_results_glmer_ranef$PrevRaceHorse.JyoType1Cd)[row], const_case_when_single_quote)
		
		# Col : THEN
		var_ranef_PrevRaceHorse.JyoType1Cd_line_then <- const_case_then
		for(col in 1: var_ranef_PrevRaceHorse.JyoType1Cd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$PrevRaceHorse.JyoType1Cd[col]),nomatch=0) > 0)
			{
				var_ranef_PrevRaceHorse.JyoType1Cd_line_name <- colnames(jackpot_results_glmer_ranef$PrevRaceHorse.JyoType1Cd[col])
				var_ranef_PrevRaceHorse.JyoType1Cd_line_value <- as.character(jackpot_results_glmer_ranef$PrevRaceHorse.JyoType1Cd[row,col])
				var_ranef_PrevRaceHorse.JyoType1Cd_line_then <- paste0(var_ranef_PrevRaceHorse.JyoType1Cd_line_then,const_ope_plus,const_convert_open,var_ranef_PrevRaceHorse.JyoType1Cd_line_name,const_convert_close,const_ope_multi,var_ranef_PrevRaceHorse.JyoType1Cd_line_value)
			}
		}
		# Col
		var_ranef_PrevRaceHorse.JyoType1Cd_line_sql <- paste(var_ranef_PrevRaceHorse.JyoType1Cd_line_when,var_ranef_PrevRaceHorse.JyoType1Cd_line_then)
		
		# Replace (Intercept -> 1)
		var_ranef_PrevRaceHorse.JyoType1Cd_line_sql <- gsub(const_intecept,const_1,var_ranef_PrevRaceHorse.JyoType1Cd_line_sql)
		# Replace (Colon -> Asterisk)
		var_ranef_PrevRaceHorse.JyoType1Cd_line_sql <- gsub(const_colon,const_asterisk,var_ranef_PrevRaceHorse.JyoType1Cd_line_sql)
		# Replace (TRUE -> Empty)
		var_ranef_PrevRaceHorse.JyoType1Cd_line_sql <- gsub(const_bool_true,const_empty,var_ranef_PrevRaceHorse.JyoType1Cd_line_sql)
		
		# Add Row
		var_ranef_PrevRaceHorse.JyoType1Cd_row_sql <- cbind(var_ranef_PrevRaceHorse.JyoType1Cd_line_sql)
		var_ranef_PrevRaceHorse.JyoType1Cd_matrix <- rbind(var_ranef_PrevRaceHorse.JyoType1Cd_matrix,var_ranef_PrevRaceHorse.JyoType1Cd_row_sql)
	}
	# ELSE NULL
	var_ranef_PrevRaceHorse.JyoType1Cd_row_sql <- cbind(const_case_else_null)
	var_ranef_PrevRaceHorse.JyoType1Cd_matrix <- rbind(var_ranef_PrevRaceHorse.JyoType1Cd_matrix,var_ranef_PrevRaceHorse.JyoType1Cd_row_sql)
	
	# END
	var_ranef_PrevRaceHorse.JyoType1Cd_row_sql <- cbind(const_case_end)
	var_ranef_PrevRaceHorse.JyoType1Cd_matrix <- rbind(var_ranef_PrevRaceHorse.JyoType1Cd_matrix,var_ranef_PrevRaceHorse.JyoType1Cd_row_sql)
	
	# SQL
	var_sql_matrix <- rbind(var_sql_matrix,var_ranef_PrevRaceHorse.JyoType1Cd_matrix)
}
#----------------------------------------------------------------------------------------------
# Random Effects (PrevRaceHorse.RaceType0Cd)
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType0Cd)>0)
{
	# Initialize
	var_ranef_PrevRaceHorse.RaceType0Cd_dim_row<-dim(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType0Cd)[1]
	var_ranef_PrevRaceHorse.RaceType0Cd_dim_col<-dim(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType0Cd)[2]
	var_ranef_PrevRaceHorse.RaceType0Cd_matrix <- NULL
	
	# CASE
	var_ranef_PrevRaceHorse.RaceType0Cd_row_sql <- cbind(const_case_case)
	var_ranef_PrevRaceHorse.RaceType0Cd_matrix <- rbind(var_ranef_PrevRaceHorse.RaceType0Cd_matrix,var_ranef_PrevRaceHorse.RaceType0Cd_row_sql)
	
	for (row in 1:var_ranef_PrevRaceHorse.RaceType0Cd_dim_row)
	{
		# Col : WHEN
		var_ranef_PrevRaceHorse.RaceType0Cd_line_when <- paste0(const_case_when_PrevRaceHorse.RaceType0Cd, rownames(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType0Cd)[row], const_case_when_single_quote)
		
		# Col : THEN
		var_ranef_PrevRaceHorse.RaceType0Cd_line_then <- const_case_then
		for(col in 1: var_ranef_PrevRaceHorse.RaceType0Cd_dim_col)
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType0Cd[col]),nomatch=0) > 0)
			{
				var_ranef_PrevRaceHorse.RaceType0Cd_line_name <- colnames(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType0Cd[col])
				var_ranef_PrevRaceHorse.RaceType0Cd_line_value <- as.character(jackpot_results_glmer_ranef$PrevRaceHorse.RaceType0Cd[row,col])
				var_ranef_PrevRaceHorse.RaceType0Cd_line_then <- paste0(var_ranef_PrevRaceHorse.RaceType0Cd_line_then,const_ope_plus,const_convert_open,var_ranef_PrevRaceHorse.RaceType0Cd_line_name,const_convert_close,const_ope_multi,var_ranef_PrevRaceHorse.RaceType0Cd_line_value)
			}
		}
		# Col
		var_ranef_PrevRaceHorse.RaceType0Cd_line_sql <- paste(var_ranef_PrevRaceHorse.RaceType0Cd_line_when,var_ranef_PrevRaceHorse.RaceType0Cd_line_then)
		
		# Replace (Intercept -> 1)
		var_ranef_PrevRaceHorse.RaceType0Cd_line_sql <- gsub(const_intecept,const_1,var_ranef_PrevRaceHorse.RaceType0Cd_line_sql)
		# Replace (Colon -> Asterisk)
		var_ranef_PrevRaceHorse.RaceType0Cd_line_sql <- gsub(const_colon,const_asterisk,var_ranef_PrevRaceHorse.RaceType0Cd_line_sql)
		# Replace (TRUE -> Empty)
		var_ranef_PrevRaceHorse.RaceType0Cd_line_sql <- gsub(const_bool_true,const_empty,var_ranef_PrevRaceHorse.RaceType0Cd_line_sql)
		
		# Add Row
		var_ranef_PrevRaceHorse.RaceType0Cd_row_sql <- cbind(var_ranef_PrevRaceHorse.RaceType0Cd_line_sql)
		var_ranef_PrevRaceHorse.RaceType0Cd_matrix <- rbind(var_ranef_PrevRaceHorse.RaceType0Cd_matrix,var_ranef_PrevRaceHorse.RaceType0Cd_row_sql)
	}
	# ELSE NULL
	var_ranef_PrevRaceHorse.RaceType0Cd_row_sql <- cbind(const_case_else_null)
	var_ranef_PrevRaceHorse.RaceType0Cd_matrix <- rbind(var_ranef_PrevRaceHorse.RaceType0Cd_matrix,var_ranef_PrevRaceHorse.RaceType0Cd_row_sql)
	
	# END
	var_ranef_PrevRaceHorse.RaceType0Cd_row_sql <- cbind(const_case_end)
	var_ranef_PrevRaceHorse.RaceType0Cd_matrix <- rbind(var_ranef_PrevRaceHorse.RaceType0Cd_matrix,var_ranef_PrevRaceHorse.RaceType0Cd_row_sql)
	
	# SQL
	var_sql_matrix <- rbind(var_sql_matrix,var_ranef_PrevRaceHorse.RaceType0Cd_matrix)
}
#----------------------------------------------------------------------------------------------
# Random Effects (AnalyzeExpectedValue01Gen02TotalAnalyze.PrevHansyokuNum)
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$AnalyzeExpectedValue01Gen02TotalAnalyze.PrevHansyokuNum)>0)
{
	# Initialize
	var_ranef_AnalyzeExpectedValue01Gen02TotalAnalyze.PrevHansyokuNum_dim_row<-dim(jackpot_results_glmer_ranef$AnalyzeExpectedValue01Gen02TotalAnalyze.PrevHansyokuNum)[1]
	var_ranef_AnalyzeExpectedValue01Gen02TotalAnalyze.PrevHansyokuNum_dim_col<-dim(jackpot_results_glmer_ranef$AnalyzeExpectedValue01Gen02TotalAnalyze.PrevHansyokuNum)[2]
	...
	{
		# Col : WHEN
		var_ranef_AnalyzeExpectedValue01Gen02TotalAnalyze.PrevHansyokuNum_line_when <- paste0(const_case_when_AnalyzeExpectedValue01Gen02TotalAnalyze.PrevHansyokuNum, sprintf("%8d",as.integer(rownames(jackpot_results_glmer_ranef$AnalyzeExpectedValue01Gen02TotalAnalyze.PrevHansyokuNum)[row])), const_case_when_single_quote)
		...
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$AnalyzeExpectedValue01Gen02TotalAnalyze.PrevHansyokuNum[col]),nomatch=0) > 0)
			{
				var_ranef_AnalyzeExpectedValue01Gen02TotalAnalyze.PrevHansyokuNum_line_name <- colnames(jackpot_results_glmer_ranef$AnalyzeExpectedValue01Gen02TotalAnalyze.PrevHansyokuNum[col])
				var_ranef_AnalyzeExpectedValue01Gen02TotalAnalyze.PrevHansyokuNum_line_value <- as.character(jackpot_results_glmer_ranef$AnalyzeExpectedValue01Gen02TotalAnalyze.PrevHansyokuNum[row,col])
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$AnalyzeExpectedValue01HorseTotalAnalyze.PrevKettoNum)>0)
{
	# Initialize
	var_ranef_AnalyzeExpectedValue01HorseTotalAnalyze.PrevKettoNum_dim_row<-dim(jackpot_results_glmer_ranef$AnalyzeExpectedValue01HorseTotalAnalyze.PrevKettoNum)[1]
	var_ranef_AnalyzeExpectedValue01HorseTotalAnalyze.PrevKettoNum_dim_col<-dim(jackpot_results_glmer_ranef$AnalyzeExpectedValue01HorseTotalAnalyze.PrevKettoNum)[2]
	...
	{
		# Col : WHEN
		var_ranef_AnalyzeExpectedValue01HorseTotalAnalyze.PrevKettoNum_line_when <- paste0(const_case_when_AnalyzeExpectedValue01HorseTotalAnalyze.PrevKettoNum, sprintf("%10d",as.integer(rownames(jackpot_results_glmer_ranef$AnalyzeExpectedValue01HorseTotalAnalyze.PrevKettoNum)[row])), const_case_when_single_quote)
		...
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$AnalyzeExpectedValue01HorseTotalAnalyze.PrevKettoNum[col]),nomatch=0) > 0)
			{
				var_ranef_AnalyzeExpectedValue01HorseTotalAnalyze.PrevKettoNum_line_name <- colnames(jackpot_results_glmer_ranef$AnalyzeExpectedValue01HorseTotalAnalyze.PrevKettoNum[col])
				var_ranef_AnalyzeExpectedValue01HorseTotalAnalyze.PrevKettoNum_line_value <- as.character(jackpot_results_glmer_ranef$AnalyzeExpectedValue01HorseTotalAnalyze.PrevKettoNum[row,col])
...
#----------------------------------------------------------------------------------------------
if (length(jackpot_results_glmer_ranef$AnalyzeExpectedValue01JockyTotalAnalyze.PrevKisyuCode)>0)
{
	# Initialize
	var_ranef_AnalyzeExpectedValue01JockyTotalAnalyze.PrevKisyuCode_dim_row<-dim(jackpot_results_glmer_ranef$AnalyzeExpectedValue01JockyTotalAnalyze.PrevKisyuCode)[1]
	var_ranef_AnalyzeExpectedValue01JockyTotalAnalyze.PrevKisyuCode_dim_col<-dim(jackpot_results_glmer_ranef$AnalyzeExpectedValue01JockyTotalAnalyze.PrevKisyuCode)[2]
	...
	{
		# Col : WHEN
		var_ranef_AnalyzeExpectedValue01JockyTotalAnalyze.PrevKisyuCode_line_when <- paste0(const_case_when_AnalyzeExpectedValue01JockyTotalAnalyze.PrevKisyuCode, sprintf("%05d",as.integer(rownames(jackpot_results_glmer_ranef$AnalyzeExpectedValue01JockyTotalAnalyze.PrevKisyuCode)[row])), const_case_when_single_quote)
		...
		{
			# if (match(const_wild_card_Intecept,colnames(jackpot_results_glmer_ranef$AnalyzeExpectedValue01JockyTotalAnalyze.PrevKisyuCode[col]),nomatch=0) > 0)
			{
				var_ranef_AnalyzeExpectedValue01JockyTotalAnalyze.PrevKisyuCode_line_name <- colnames(jackpot_results_glmer_ranef$AnalyzeExpectedValue01JockyTotalAnalyze.PrevKisyuCode[col])
				var_ranef_AnalyzeExpectedValue01JockyTotalAnalyze.PrevKisyuCode_line_value <- as.character(jackpot_results_glmer_ranef$AnalyzeExpectedValue01JockyTotalAnalyze.PrevKisyuCode[row,col])
...
if (length(jackpot_results_glmer_ranef$AnalyzeExpectedValue01Gen02TotalAnalyze)>0)
{
# SQL
...
if (length(jackpot_results_glmer_ranef$AnalyzeExpectedValue01HorseTotalAnalyze)>0)
{
# SQL
...
if (length(jackpot_results_glmer_ranef$AnalyzeExpectedValue01JockyTotalAnalyze)>0)
{
# SQL
