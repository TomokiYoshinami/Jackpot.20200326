...
# Log Start
# ---------------------------------------------------------------------------------------------------------------------------
logFile <- (paste0("log_Predict_",  jackpot_analyzeType, "_Type", jackpot_trackTypeCd, jackpot_jyokenCd,".log"))
sink(logFile, append=F, split=T)
print(paste0(Sys.time(),  " --- ", jackpot_version, "_", jackpot_analyzeType, "_", jackpot_analyzeFamily, "_", jackpot_rType, "_Type", jackpot_trackTypeCd, jackpot_jyokenCd, " Start ---"))
print("--- Log Start ---")
# ---------------------------------------------------------------------------------------------------------------------------
# Environment
# ---------------------------------------------------------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------------------------------------------------------
print("--- Load Start ---")
load(paste0("RData_",  jackpot_rType , "_Type", jackpot_trackTypeCd, jackpot_jyokenCd,".RData"))
print("--- Load Finished ---")
# ---------------------------------------------------------------------------------------------------------------------------
# InputDataSet
# ---------------------------------------------------------------------------------------------------------------------------
print(paste0("nrow(InputDataSet)=", nrow(InputDataSet)))
...
# ---------------------------------------------------------------------------------------------------------------------------
###############################################################
# InputDataSet <- subset(InputDataSet, Race.IsAnalyzeFullPastAndFuture == 1 & Race.jackpot_trackTypeCd == jackpot_trackTypeCd & Race.jackpot_jyokenCd == jackpot_jyokenCd)
###############################################################
print(paste0("class(InputDataSet$Race.IsAnalyzeNormalPast)=", class(InputDataSet$Race.IsAnalyzeNormalPast)))
print(paste0("class(InputDataSet$Race.jackpot_trackTypeCd)=", class(InputDataSet$Race.jackpot_trackTypeCd)))
print(paste0("nrow(InputDataSet) subset after=", nrow(InputDataSet)))
# ---------------------------------------------------------------------------------------------------------------------------
# InputDataSet transform
# ---------------------------------------------------------------------------------------------------------------------------
print("--- Transform Start ---")
# source(paste0("source_transform_predict.", jackpot_analyzeType, ".R"))
print("--- Transform Finished ---")
print(paste0("nrow(InputDataSet) transaform after=", nrow(InputDataSet)))
print("str(InputDataSet)=")
print(str(InputDataSet, list.len=ncol(InputDataSet)))
# print("str(InputDataSet)")
# print(str(InputDataSet, list.len=ncol(InputDataSet)))
...
# ---------------------------------------------------------------------------------------------------------------------------
jackpot_InputDataSet_before <- nrow(InputDataSet)
InputDataSet <- na.omit(InputDataSet)
jackpot_InputDataSet_after <- nrow(InputDataSet)
jackpot_InputDataSet_diff <- jackpot_InputDataSet_before - jackpot_InputDataSet_after
print(paste0("nrow(InputDataSet) na.omit diff #1=" , jackpot_InputDataSet_diff))
# ---------------------------------------------------------------------------------------------------------------------------
# Predict Start
# ---------------------------------------------------------------------------------------------------------------------------
print("--- Predict Start ---")
...
jackpot_InputDataSet_after <- nrow(InputDataSet)
jackpot_InputDataSet_diff <- jackpot_InputDataSet_before - jackpot_InputDataSet_after
print(paste0("InputDataSet.diff na.omit after #2=" , jackpot_InputDataSet_diff))
# ---------------------------------------------------------------------------------------------------------------------------
# OutputDataSet
# ---------------------------------------------------------------------------------------------------------------------------
...
if (jackpot_rType  == "glm")
{
# OutputDataSet$PredictValue <- predict(jackpot_results_glm_step, newdata=InputDataSet, allow.new.levels=T)
OutputDataSet$PredictValue <- predict(jackpot_results_glm_step, newdata=InputDataSet, na.action=na.omit, allow.new.levels=T)
...
if (jackpot_rType  == "glmer")
{
# OutputDataSet$PredictValue <- predict(jackpot_results_glmer, newdata=InputDataSet, allow.new.levels=T)
OutputDataSet$PredictValue <- predict(jackpot_results_glmer, newdata=InputDataSet, na.action=na.omit, allow.new.levels=T)
# na.action={na.omit, na.fail, na.exclude, na.pass}
}
# ---------------------------------------------------------------------------------------------------------------------------
# Predict End
...
print(paste0("nrow(OutputDataSet) #2=",nrow(OutputDataSet)))
# ---------------------------------------------------------------------------------------------------------------------------
# Log
# ---------------------------------------------------------------------------------------------------------------------------
print("--- Log Finished ---")
print(paste0(Sys.time(),  " --- ", jackpot_version, "_", jackpot_analyzeType, "_", jackpot_analyzeFamily, "_", jackpot_rType, "_Type", jackpot_trackTypeCd, jackpot_jyokenCd, " Finished---"))
sink()
