...
# ---------------------------------------------------------------------------------------------------------------------------
sqlViewName1 <- "ViewAnalyzeD1110Analyze01"
sqlViewName2 <- "Multi"
...
sqlQueryContext <- paste0("SELECT * FROM ", sqlViewName)

# RevoSQLR

# Define the connection string
# connStr <- "Driver=SQL Server; Server=HERZO01; Database=WideWorldImportersDW; Trusted_Connection = True"
# Set ComputeContext
sqlShareDir <- paste("C:\\Jackpot\\",Sys.getenv("USERNAME"),sep="")
sqlWait <- TRUE
sqlConsoleOutput <- FALSE
# cc <- RxInSqlServer(connectionString = connectionString, shareDir = sqlShareDir, wait = sqlWait, consoleOutput = sqlConsoleOutput)
# rxSetComputeContext(cc)

inData <- RxSqlServerData(sqlQuery = sqlQueryContext, connectionString = connectionString, rowsPerRead=500)
 # RxSqlServerData(
	# table = NULL,
	# sqlQuery = NULL,
...
print(paste0("connectionString=", connectionString))
print(paste0("sqlQueryContext=", sqlQueryContext))

str(inData)

# data < rxImport(inData = inData)
# str(data)

varInfo <- rxGetVarInfo(data = inData)
varInfo
