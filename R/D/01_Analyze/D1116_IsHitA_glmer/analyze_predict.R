...
# InputDataSet$PrevRaceHorse.IsHitA[PrevRaceHorse.IsHitA.Index] <- NA
# ---------------------------------------------------------------------------------------------------------------------------
# InputDataSet New Levels: Gen01.HansyokuNum
# ---------------------------------------------------------------------------------------------------------------------------
if (analyzeType == "Gen01")
{
	# print(paste0("levels(data$Gen01.HansyokuNum)=", levels(data$Gen01.HansyokuNum)))
	# print(paste0("levels(InputDataSet$Gen01.HansyokuNum)=", levels(InputDataSet$Gen01.HansyokuNum)))
	# Gen01.HansyokuNum.Index <- which(!(InputDataSet$Gen01.HansyokuNum %in% levels(data$Gen01.HansyokuNum)))
	# print(paste0("length(Gen01.HansyokuNum.Index)=", length(Gen01.HansyokuNum.Index)))
	# InputDataSet$Gen01.HansyokuNum[Gen01.HansyokuNum.Index] <- NA
}
# ---------------------------------------------------------------------------------------------------------------------------
# InputDataSet New Levels: Gen02.HansyokuNum
# ---------------------------------------------------------------------------------------------------------------------------
if (analyzeType == "Gen02")
{
	# print(paste0("levels(data$Gen02.HansyokuNum)=", levels(data$Gen02.HansyokuNum)))
	# print(paste0("levels(InputDataSet$Gen02.HansyokuNum)=", levels(InputDataSet$Gen02.HansyokuNum)))
	# Gen02.HansyokuNum.Index <- which(!(InputDataSet$Gen02.HansyokuNum %in% levels(data$Gen02.HansyokuNum)))
	# print(paste0("length(Gen02.HansyokuNum.Index)=", length(Gen02.HansyokuNum.Index)))
	# InputDataSet$Gen02.HansyokuNum[Gen02.HansyokuNum.Index] <- NA
}
# ---------------------------------------------------------------------------------------------------------------------------
# InputDataSet New Levels: Horse.PrevKettoNum
# ---------------------------------------------------------------------------------------------------------------------------
if (analyzeType == "Horse")
{
	# print(paste0("levels(data$Horse.PrevKettoNum)=", levels(data$Horse.PrevKettoNum)))
	# print(paste0("levels(InputDataSet$Horse.PrevKettoNum)=", levels(InputDataSet$Horse.PrevKettoNum)))
	# Horse.PrevKettoNum.Index <- which(!(InputDataSet$Horse.PrevKettoNum %in% levels(data$Horse.PrevKettoNum)))
	# print(paste0("length(Horse.PrevKettoNum.Index)=", length(Horse.PrevKettoNum.Index)))
	# InputDataSet$Horse.PrevKettoNum[Horse.PrevKettoNum.Index] <- NA
}
# ---------------------------------------------------------------------------------------------------------------------------
# InputDataSet New Levels: Jockey.KisyuCode
# ---------------------------------------------------------------------------------------------------------------------------
if (analyzeType == "Jockey")
{
	# print(paste0("levels(data$Jockey.KisyuCode)=", levels(data$Jockey.KisyuCode)))
	# print(paste0("levels(InputDataSet$Jockey.KisyuCode)=", levels(InputDataSet$Jockey.KisyuCode)))
	# Jockey.KisyuCode.Index <- which(!(InputDataSet$Jockey.KisyuCode %in% levels(data$Jockey.KisyuCode)))
	# print(paste0("length(Jockey.KisyuCode.Index)=", length(Jockey.KisyuCode.Index)))
	# InputDataSet$Jockey.KisyuCode[Jockey.KisyuCode.Index] <- NA
}
# ---------------------------------------------------------------------------------------------------------------------------
# InputDataSet New Levels
# ---------------------------------------------------------------------------------------------------------------------------
...
# print(paste0("analyzeFunc=", analyzeFunc))
# print("analyze.results.glmerer=")
# print(analyze.results.glmerer)
# print("analyze.results.glmerer.fixef=")
# print(analyze.results.glmerer.fixef)
# print("analyze.results.glmerer_ranef=")
# print(analyze.results.glmerer_ranef)
# print("analyze.results.glmerer.summary=")
# print(analyze.results.glmerer.summary)
# print("analyze.results.glmerer_vif =)
# print(analyze.results.glmerer_vif)

# OutputDataSet$PredictValue <- predict(analyze.results.glmerer, newdata=InputDataSet, na.action=na.omit, allow.new.levels=TRUE)
...
# rxPredict
# ---------------------------------------------------------------------------------------------------------------------------
print(paste0(Sys.time(), " --- rxPredict (glmer) Start ---"))
{
print(paste0("analyzeFunc=", analyzeFunc))

	if(nrow.InputDataSet.predict > 0)
	{
	PredictDataSet <- data.frame(rxPredict(analyze.results.glmer.step, data = InputDataSet, writeModelVars = FALSE))
	}
...
# na.action={na.omit, na.fail, na.exclude, na.pass}
print(paste0(Sys.time(), " --- rxPredict (glmer) Finish ---"))
}
